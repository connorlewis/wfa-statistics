...
temp.comp.dta <- reactive({
  temp.df <- temp.last.dta() %>%
                select(plyr.lbl, Team = team, Games = game, Rating = qb.rate.cum
                       , Attempts = Att.cum
...
temp.rb.comp.dta <- reactive({
  temp.df <- temp.rb.last.dta() %>%
                select(plyr.lbl, Team = team, Games = game, Carries = car.cum
...
temp.wr.comp.dta <- reactive({
  temp.df <- temp.wr.last.dta() %>%
                select(plyr.lbl, Team = team, Games = game, Receptions = rec.cum
                  , TD = TD.cum, `Avg Yds` = avg.cum, `Yds per Game` = yds.cum.game) %>% 
      t
...
datatable(pass.sum.dta %>% 
  group_by(Player) %>%
          top_n(1, as.numeric(game)) %>%
          select(Player, Team = team, Games = game, Rating = qb.rate.cum, Attempts = Att.cum
...
datatable(rush.sum.dta %>% 
  group_by(Player) %>%
          top_n(1, as.numeric(game)) %>%
          select(Player, Team = team, Games = game, Carries = car.cum
...
datatable(rec.sum.dta %>% 
  group_by(Player) %>%
          top_n(1, as.numeric(game)) %>%
          select(Player, Team = team, Games = game, Recs = rec.cum
                 , Yds = yds.cum, Avg = avg.cum
                 , TD = TD.cum, `Yds per Game` = yds.cum.game) %>%
          arrange(desc(as.numeric(Yds)))
  , options = list(bPaginate = FALSE, pageLength = 10), fillContainer = TRUE)
