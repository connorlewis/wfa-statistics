...
renderPlot({
   ...
   temp.dta() %>%
    ...
    geom_text(data = temp.last.dta(),
              mapping = aes(x = game - 0.8, y = TD.Int.cum, label = plyr.lbl, color = Player))
...
renderPlot({
   ...
   temp.dta() %>%
    ggplot(aes(x = game, y = qb.rate.cum
               , group = Player, color = Player)) +
    
       geom_line(data = pass.sum.dta 
            , mapping = aes(x = game, y = qb.rate.cum
                            , group = factor(Player))
            , color = "grey") +
       geom_smooth(data = pass.sum.dta 
                , mapping = aes(x = game, y = qb.rate.cum)
    ...
    scale_color_discrete(guide = FALSE) +
    geom_text(data = temp.last.dta(),
              mapping = aes(x = game - 0.8, y = qb.rate.cum, label = plyr.lbl, color = Player))
