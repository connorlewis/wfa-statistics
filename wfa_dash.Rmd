...
source("data_prep.R")
#qb.dash.dta <- read_csv("PassingDataTemp.csv")
...
renderPlot({
    ...
    geom_point(data = qb.dash.dta %>%
                group_by(player) %>%
                filter(player != input$qb) %>%
                top_n(1, week)
                , mapping = aes(x = att_cum, y = yards_cum)
                , color = "grey") +
    geom_smooth(data = qb.dash.dta %>%
                group_by(player) %>%
                top_n(1, week)
                , mapping = aes(x = att_cum, y = yards_cum)
...
```{r td.int.plot}
renderPlot({
  var.by.game.plot(qb.dta(), qb.dash.dta
...
```{r rush.yrds.game}
renderPlot({
  var.by.game.plot(rb.dta(), rb.dash.dta
...
```{r}
renderPlot({
  var.by.game.plot(wr.dta(), wr.dash.dta
